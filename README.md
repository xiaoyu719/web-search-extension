# 联网搜索

SillyTavern / TauriTavern 通用前端扩展。不绑角色卡。

你发消息之后、AI 写正文之前，它会按规则决定要不要上网抓一段公开摘要，再塞进提示词。同人、原作、真人真事默认当「不能瞎编」：先判断、先搜索，再写故事。

> 文件写好 ≠ 已经在你的酒馆里生效。真机安装、试搜、发消息都还没验过。

## 用 GitHub 地址安装（社区常用）

仓库根目录必须有 `manifest.json`。别人装的时候，把**仓库主页地址**贴进酒馆即可，不必再复制文件夹。

### SillyTavern

1. 本机先装好 Git。
2. 点顶栏扩展（拼图）→ **Install Extension / 安装扩展**。
3. 粘贴仓库地址，地址：`https://github.com/xiaoyu719/web-search-extension`
4. 可选填分支，多人模式可选装给自己还是所有人。
5. 安装后启用「联网搜索」，刷新一次。

官方说明：[Extensions](https://docs.sillytavern.app/extensions/)

### TauriTavern（Tauri 酒馆）

也可以用同一套前端扩展、同一条「粘贴 Git 地址」安装。

- 只接受匿名 `http(s)` Git 地址（GitHub / GitLab / Gitee 都可以）。
- 不要用 SSH、不要带登录信息、不要贴网页树状浏览那种带多余参数的链接。
- **不支持** Node 后端插件；本扩展本来就是纯前端，这一点是对齐的。
- 跨域限制仍和浏览器酒馆同类：大陆默认走求闻百科 / 萌娘百科；维基百科、DuckDuckGo、Tavily 在大陆可能连不上或被拦。

### 也可以手动复制

把整个 `web-search-extension` 文件夹复制到：

- SillyTavern：`data/<用户名>/extensions/web-search-extension/`
- 或：`public/scripts/extensions/third-party/web-search-extension/`
- TauriTavern：第三方扩展目录，文件夹名保持 `web-search-extension`

## 上传 GitHub 时要注意

- 仓库名建议就叫 `web-search-extension`，根目录直接放 `manifest.json`，不要再套一层文件夹。
- 公开仓库才能让别人用「粘贴地址」安装。
- 许可证是 MIT：可以玩、可以改、可以二创，保留署名即可。本扩展不依赖 Server Plugin。
- 仓库地址：https://github.com/xiaoyu719/web-search-extension
- 密钥只存在本机扩展设置，不要写进仓库。

## 它会做什么

- **同人/不可原创**词命中：搜。
- **原创/架空**词命中：不搜。
- 近况、比分、新闻、资料问题：搜。
- 「嗯」「继续」和纯扮演：不搜。
- 打开「生成前先判断」时，会先做一次聊天里看不见的短询问，需要就先搜再写正文。回复会稍慢。
- `/websearch 关键词` 仍可强制搜。

## 搜索源

## 主API 要不要接？

不需要再申请一套搜索 Key。求闻百科 / 萌娘百科是网站公开查询口，插件自动去问。

你聊天用的那套主 API（模型接口）也不是搜百科的必要条件。只有网页百科抓不到、或你主动选「只跟随主API」时，才会用你当前已连上的聊天接口再问一轮。这不另配地址和密钥。

注意：主API 走的是模型。它若不能上网，这不是真正的网页搜索。


大陆默认（本机测过：接口能打开，并且浏览器跨域头允许抓取）：

- 求闻百科（综合百科，适合人物、赛事、地名等）
- 萌娘百科（二次元 / 同人资料更多）

海外备用（默认关闭；大陆常连不上，勾上后可能空等超时）：

- 维基百科中文 / Wikipedia 英文
- DuckDuckGo 即时答案

其它：

- 自定义地址（自建 SearXNG / 代理，用 `{{query}}` 占位）
- 可选 Tavily：密钥只保存在本机

百度百科、搜狗百科、360 百科：网页在大陆一般能打开，但接口不给浏览器跨域头，插件**抓不到**，所以没有做成默认源。要用只能自己填可跨域的代理地址。

**不能保证**每一家运营商、每一种酒馆窗口都免 VPN。文件写好 ≠ 你电脑上已经搜通。

## 搜索结果写到哪里

神数据库「召回/补充」里的补充栏，公开接口写不进去；不加自建表也写不进去。所以 1.0.6 改成写入**世界书条目**，兼容性更好。

- 条目备注以 `[联网搜索]` 开头。只改这类条目，不碰你原来的世界书内容。
- 同一检索词会覆盖更新，不会每轮新开一条。
- 默认写当前角色的主世界书；没有就用已有的第一本。不会偷偷新建世界书。
- 角色还没绑世界书时会跳过，搜索本身照常。
- 这不是神数据库的补充栏。真机写入尚未验证。
- 世界书里已经有对应内容时，默认不再搜索、也不再让模型先判断，省 token。你主动说「搜一下 / 最新」或用 `/websearch` 仍会搜。

## 还没在真机验证

- 扩展管理里看见、启用、禁用
- 用 GitHub 地址安装与更新
- 试搜、带关键词发消息、同人先搜再写
- Tauri 酒馆晚加载时拦截器是否仍工作

## 明确不做

- 不替你装进正在用的酒馆
- 不写 Node 后端插件
- 不把密钥写进角色卡
- 不修改足球人生
